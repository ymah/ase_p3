...
create_ifile(enum file_type_e type) 
{
    ...
    if(DEBUG)
      printDebug(FNNAME, "create inode");
...
flush_ifile(file_desc_t *fd)
{
	...
	vbloc = allocate_vbloc_of_fbloc(fd-> fds_inumber, fbloc);
...
writec_ifile(file_desc_t *fd, char c)
{
    ...
    if (! fd->fds_dirty) {
        ibloc = allocate_vbloc_of_fbloc(fd->fds_inumber, bloc_of_pos(fd->fds_pos));
        if (! ibloc) 
	...
	/* write the buffer */
        ibloc = vbloc_of_fbloc(fd->fds_inumber, bloc_of_pos(fd->fds_pos));
	write_bloc(current_vol, ibloc, fd->fds_buf);
	/* read the new buffer */
	ibloc = vbloc_of_fbloc(fd->fds_inumber,
                               bloc_of_pos(fd->fds_pos+1));
	if (! ibloc) 
	    memset(fd->fds_buf, 0, BLOC_SIZE);
	else
